...
(function ($) {
    ...
    FileInput.prototype = {
        ...
        initFileActions: function () {
            var self = this;
            self.$preview.find('.kv-file-remove:not(.customBtn)').each(function () {
        ...
        renderFileFooter: function (caption, width) {
            ...
            if (self.isUploadable && self.showAction) {
        ...
        initPreviewDeletes: function () {
            var self = this, deleteExtraData = self.deleteExtraData || {},
                resetProgress = function () {
                    if (self.$preview.find('.kv-file-remove:not(.customBtn)').length === 0) {
                        self.reset();
                    }
                };
            self.$preview.find('.kv-file-remove:not(.customBtn)').each(function () {
        ...
        cancel: function () {
            ...
            self.$preview.find(PREVIEW_FRAMES).each(function () {
                ...
                if (self.filestack[ind] !== undefined) {
                    ...
                    $thumb.find('.kv-file-remove:not(.customBtn)').removeClass('disabled').removeAttr('disabled');
                }
                self.unlock();
            });
        },
        ...
        uploadSingle: function (i, files, allFiles) {
            var self = this, total = self.getFileStack().length, formdata = new FormData(), outData,
                ...
                pct, chkComplete, $btnUpload = $thumb.find('.kv-file-upload'), $btnDelete = $thumb.find('.kv-file-remove:not(.customBtn)'),
        ...
        uploadBatch: function () {
            ...
            enableActions = function (i) {
                ...
                $thumb.removeClass('file-uploading');
                $thumb.addClass('file-preview-initial');
                $btnUpload.removeAttr('disabled');
                $btnDelete.removeAttr('disabled');
            };
            ...
            fnBefore = function (jqXHR) {
                ...
                if (self.showPreview) {
                    self.$preview.find(PREVIEW_FRAMES).each(function () {
                        var $thumb = $(this), $btnUpload = $thumb.find('.kv-file-upload'), $btnDelete = $thumb.find('.kv-file-remove:not(.customBtn)');
                        addCss($thumb, 'file-uploading');
                        $btnUpload.attr('disabled', true);
                        $btnDelete.attr('disabled', true);
                    });
            ...
            fnSuccess = function (data, textStatus, jqXHR) {
                ...
                if (data.error === undefined || isEmpty(data.error)) {
                    ...
                    if (self.showPreview) {
                        ...
                        self.$preview.find('.kv-file-remove:not(.customBtn)').hide();
                        self.$preview.find(PREVIEW_FRAMES).each(function () {
                            ...
                            if(data.success)
                            {
                                var deleteBtnHtml =  "<button id=\"deleteBtn_" + (self.previewInitId + "-" + key) + "\" type=\"button\" class=\"kv-file-remove btn btn-xs btn-default customBtn\" dataJson='" + JSON.stringify(data) + "' previewInitId='" + (self.previewInitId + "-" + key) + "' title=\"删除文件\" >";
                                    deleteBtnHtml +=     "<i class=\"glyphicon glyphicon-trash text-danger\"></i>";
                                    deleteBtnHtml += "</button>";
                                $("#" + (self.previewInitId + "-" + key)).find(".file-footer-buttons").append(deleteBtnHtml);
                                //添加删除时间
                                var customDeleteBtn = $("#deleteBtn_" + (self.previewInitId + "-" + key));
                                customDeleteBtn.click(function(){
                                    self.customDeleteClick($(this));
                                });
                                //重新初始化id避免id不变造成divId重复现象
                                self.previewInitId = "preview-" + uniqId();
                            }
                        });
                    ...
                    if (self.showPreview) {
                        self.$preview.find(PREVIEW_FRAMES).each(function () {
                            ...
                            if ($.inArray(key, keys) !== -1) {
                                ...
                                $thumb.find('.kv-file-remove:not(.customBtn)').hide();
                                setIndicator(key, 'indicatorSuccess', 'indicatorSuccessTitle');
                                self.filestack[key] = undefined;
                            }
                        });
        ...
        previewFile: function (file, theFile, previewId, data) {
            ...
            if (chkTypes && chkMimes) {
                ...
                if(self.maxFileCount == 1)
                {
                    if(self.$preview.find(".file-preview-frame.file-preview-initial").length > 0)
                    {
                        self.$preview.find(".file-preview-frame.file-preview-initial").remove();
                    }
                }
                self.$preview.append("\n" + content);
        ...
        customDeleteClick: function (btn) {
            var _dataJson = $(btn).attr("dataJson");
            var _dataPreviewInitId = $(btn).attr("previewInitId");
            var _dataInfo = {
                data: JSON.parse(_dataJson),
                previewInitId: _dataPreviewInitId
            };
            var self = this;
            self.raise('customDelete', _dataInfo);
        },
        removePreviewInfo:function (previewInitId) {
